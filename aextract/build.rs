use std::{
    env, fs, path::Path, process::Command
};
...
impl Platform {
    pub fn new(id: impl AsRef<str>, dylib: impl AsRef<str>, runtime: impl AsRef<str>, executable: impl AsRef<str>) -> Self {
        Self {
    ...
    pub fn lib_filename(&self) -> String {
        Url::parse(&self.lib_url()).unwrap().path().rsplit_once("/").unwrap().1.to_string()
    }

    pub fn publish(&self) -> () {
        fs::create_dir_all(Path::new(&env::var_os("OUT_DIR").unwrap())
                    .join("binaries")
                    .join(self.id.clone())).expect("Should be able to create binary output directory.");
    ...
    pub fn download_lib(&self) -> () {
        if !Path::new(&env::var_os("OUT_DIR").unwrap()).join("shared_libs").exists() {
            fs::create_dir_all(Path::new(&env::var_os("OUT_DIR").unwrap()).join("shared_libs")).expect("Should be able to create shared library output directory.");
        }
        let mut req = reqwest::blocking::get(self.lib_url()).expect("Failed to request dylib.");
        let mut dl_file = fs::File::create(Path::new(&env::var_os("OUT_DIR").unwrap()).join("shared_libs").join(self.lib_filename())).unwrap();
    ...
    pub fn create_getter(&self) -> () {
        ...
        fs::write(Path::new(&env::var_os("OUT_DIR").unwrap()).join("binaries.rs"), formatted).unwrap();
    }
}
...
fn get_platforms() -> Vec<Platform> {
    ...
    platforms.push(Platform::new("linux_x86_64", "linux/lib/liboo2corelinux64.so.9", "linux-x64", "AextractSidecar"));

    #[cfg(all(target_os = "linux", target_arch = "aarch64"))]
    platforms.push(Platform::new("linux_aarch64", "linuxarm/lib/liboo2corelinuxarm64.so.9", "linux-arm64", "AextractSidecar"));

    #[cfg(all(target_os = "windows", target_arch = "x86_64"))]
    platforms.push(Platform::new("windows_x86_64", "win/redist/oo2core_9_win64.dll", "win-x64", "AextractSidecar.exe"));

    #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
    platforms.push(Platform::new("macos_x86_64", "mac/lib/liboo2coremac64.2.9.10.dylib", "osx-x64", "AextractSidecar"));
